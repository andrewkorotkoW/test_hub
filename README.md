...
просит светлую — светлая, если явно просит тёмную или вообще не сообщает
предпочтение — тёмная (тёмная тема выбрана дефолтом продукта, а не только
запасным вариантом). Тема выставляется на `<html>` сразу при загрузке
`common.js`, ещё до отрисовки сайдбара — это работает и на страницах без
сайдбара (`index.html`, `share.html`), чтобы при переходе между страницами не
было вспышки не той темы.

Токены темы — CSS-переменные в `ui/style.css`: не зависящие от темы
(`--accent`, `--danger`, `--radius`, `--card-radius`, статус-цвета — см. ниже)
заданы в `:root`, зависящие от темы (`--bg`, `--bg-sidebar`, `--surface`,
`--surface-hover`, `--border`, `--text`, `--text-muted`, `--shadow-card`) —
дважды, под `:root, [data-theme="dark"]` (тёмная — значение по умолчанию, пока
атрибут ещё не проставлен) и под `[data-theme="light"]`. Отдельно заведён
неоновый градиент фиолетовый→розовый→голубой (`--gradient-start`/`-mid`/`-end`,
`--gradient-neon`) для площадных графиков и колец дашборда.
