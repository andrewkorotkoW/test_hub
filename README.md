...
выше), только вручную с подтверждением. Для `auto_tests_vshgu_cloude` стенд
...
На странице проекта под деревом тестов для такого стенда вместо обычных кнопок
...
подтверждения — что для дерева тестов, что для пресета (обработчик общий).
