...
`PROJECT_COLOR_PALETTE` — синий, фиолетовый, розовый, голубой, зелёный,
оранжевый, красный, бирюзовый, серый; одинаковый список продублирован в
`app/schemas.py` и `ui/common.js`, порядок и значения совпадают). Хранится в
колонке `projects.color` (миграция `_migrate_add_column` в `app/db.py`); пока
