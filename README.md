...
проекта, сгруппированных по областям продукта. Цвет ячейки маршрута — сколько
тестов его покрывает (чем больше тестов, тем насыщеннее цвет), серая заливка —
тестов нет вообще; красная обводка — на выбранном стенде последний прогон,
затрагивающий этот маршрут, упал. Переключатель стенда (develop/stage) меняет,
для какого стенда подсвечиваются упавшие маршруты и считается сводка. Есть
фильтры «только непокрытые», «только упавшие» и поиск по пути маршрута.

Клик по ячейке маршрута открывает справа панель связей — список тестов, которые
его покрывают, с окружением и статусом на каждом стенде. Клик по тесту в этом
списке работает в обратную сторону: список маршрутов и UI-страниц, которые
дёргает этот тест, с подсветкой соответствующих ячеек на карте. Если маршрут
покрывают несколько тестов, на его ячейке рядом с методом и путём подписано их
число.

Сверху — сводка: по каждому стенду сколько маршрутов всего/покрыто и процент, а
также список областей продукта без единого покрытого маршрута ни на одном
стенде. Кнопка «Выгрузить CSV» сохраняет текущую карту (маршрут, метод,
область, покрыт ли, число тестов, статус по каждому стенду) файлом на диск.
...
3. **Покрытие (реализовано):** страница «Покрытие» — карта API-маршрутов проекта
   по областям продукта, цвет ячейки по числу покрывающих тестов, панель связей
   маршрут↔тест, сводка по стендам, выгрузка в CSV; см. «Покрытие» ниже.
